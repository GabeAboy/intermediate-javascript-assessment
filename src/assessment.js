...
// Which function(s) access the playball variable and get homerun
...
// Make a constructor function called taco that takes in 3 parameters: shell, meat, veggies and assigns them to identically named properties.
 function Taco(shell,meat,veggies) {//By convention we capitalize the first letter
   this.shell = shell;
   this.meat = meat;
...
// #9  ###################
// # Prototype 2
// Write a constructor function called chimichanga.  It has a property called percentLeft = 100.  It has a prototype function called eat.  When eat is invoked it uses context to
//implicitly subract 20 from the precentLeft on the chimichanga.
...
function subway(personName) {
  ...
  return function addIngredient(food) {
    obj.ingredients.push(food);
    return(obj);
  };
...
function compareValues(one,two) {
  if(one==two){
    if(typeof one === typeof two)return 'Exact match';
    else return 'Different types';
  }
